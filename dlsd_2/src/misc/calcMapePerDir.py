...
from dlsd_2.src.calc import error
...
def main():
    ...
    for dirname, dirlist, filelist in os.walk(args.inputDir):
        ...
        if ("output" in dirname):
            ...
            mapesForDir.to_csv(os.path.join(parentDir, "avgMapesForSensor_" + sensorId + ".csv"), index=True,
                               header=True)
    ...
    for key in sensorsInfo:
        ...
        avgMapeOfMonths.to_csv(os.path.join(sensorsInfo[key]['path'], "allMapesForSensor_" + key + ".csv"), header=True,
                               index=True)
