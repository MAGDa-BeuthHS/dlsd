...
def main():
    ...
    topMin = np.array(
        [(lambda x: df_min.iloc[:, x].astype('category').describe()[2])(col) for col in range(0, df_min.shape[1])])
    topMax = np.array(
        [(lambda x: df_max.iloc[:, x].astype('category').describe()[2])(col) for col in range(0, df_max.shape[1])])
    freq_min = np.array(
        [(lambda x: df_min.iloc[:, x].astype('category').describe()[3])(col) for col in range(0, df_min.shape[1])])
    freq_max = np.array(
        [(lambda x: df_max.iloc[:, x].astype('category').describe()[3])(col) for col in range(0, df_max.shape[1])])
