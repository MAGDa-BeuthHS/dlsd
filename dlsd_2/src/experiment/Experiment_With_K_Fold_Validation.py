...
from dlsd_2.src.io.dataset.Dataset_With_Time_Offset import Dataset_With_Time_Offset
...
class Experiment_With_K_Fold_Validation(Experiment):
    ...
    def _iterate_over_k_fold_validation(self):
        ...
        for k in range(self.k):
            self.current_k = k
            self.validation_input_target_maker = copy.deepcopy(
                self.original_validation_input_target_maker)  # normalizer from training data
    ...
    def _normalize_training_data(self):
        ...
        self.train_input_and_target_maker.target_maker.dataset_object.set_denormalizer(
            self.train_input_and_target_maker.denormalizer_used_in_training)
    ...
    def _set_denormalizer_used_in_training_and_denormalize_itm(self, itm):
        ...
        itm.target_maker.dataset_object.set_denormalizer(
            self.train_input_and_target_maker.denormalizer_used_in_training)
        itm.target_maker.dataset_object.normalize()

    def _write_target_data_to_file(self):
        self._write_itm_target_data_to_file(self.test_input_and_target_maker,
                                            self.current_experiment_helper.get_target_file_path())
        self._write_itm_target_data_to_file(self.train_input_and_target_maker,
                                            self.current_experiment_helper.get_train_target_file_path())
        self._write_itm_target_data_to_file(self.validation_input_target_maker,
                                            self.current_experiment_helper.get_validation_target_file_path())
    ...
    def _train_and_test_single_model(self, model):
        ...
        self._test_model_with_input_target_maker(model, self.test_input_and_target_maker,
                                                 self.current_experiment_helper.make_new_model_prediction_file_path_with_model_name)
        self._test_model_with_input_target_maker(model, self.train_input_and_target_maker,
                                                 self.current_experiment_helper.make_new_model_train_prediction_file_path_with_model_name)
        self._test_model_with_input_target_maker(model, self.validation_input_target_maker,
                                                 self.current_experiment_helper.make_new_model_validation_prediction_file_path_with_model_name)
