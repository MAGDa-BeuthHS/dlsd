...
class Average_Week_Content(Model_Content):
    ...
    def set_source_weekday_begin_int(self):
        ...
        self.target_begin_weekday_int = get_weekday_int_from_timestamp_string_with_format(first_day_timestamp_string,
                                                                                          self.input_target_maker.time_format)
        self.target_begin_time_int = get_time_int_from_timestamp_string_with_format(first_day_timestamp_string,
                                                                                    self.input_target_maker.time_format,
    ...
    def create_average_week_with_source_maker(self, source_maker):
        ...
        current_weekday_start_int = get_weekday_int_from_timestamp_string_with_format(source.df.index.values[0],
                                                                                      source_maker.time_format_train) if self.weekday_int_source is None else self.weekday_int_source
        average_data = rearrange_week_starting_to_start_on_monday_with_current_day_start_int(average_data,
                                                                                             current_weekday_start_int)
    ...
    def _get_idx_first_midnight(self, source_maker):
        idx = 0
        the_datetime = convert_string_to_datetime(source_maker.all_data.df.index.values[idx],
                                                  source_maker.time_format_train)
    ...
    def _extract_sensors_currently_being_used_as_output_from_source_data(self):
        self.subset_avg_week_array = self.source_average_dataset_object.df[
    ...
    def _fill_prediction_with_copies_of_avg_week_at_timeoffset_index(self, avg_week_starting_at_time,
                                                                     idx_current_time_offset, array):
        '''
