...
from dlsd_2.src.model.types.neural_networks.Neural_Network_Model import Neural_Network_Model
...
class NN_One_Hidden_Layer(Neural_Network_Model):
    ...
    def _build_model(self):
        ...
        with self.graph.as_default():
            self.input_pl = tf.placeholder(tf.float32, shape=[None, self.model_input.get_number_inputs()],
                                           name="input_placeholder")
            self.target_pl = tf.placeholder(tf.float32, shape=[None, self.model_input.get_number_targets()],
