...
class Neural_Network_Model(Model):
    ...
    def _iterate_over_data_in_batches(self, sess):
        ...
        for step in range(num_batches):
            ...
            loss_value, predicted = sess.run([self.model_content.optimize, self.model_content.prediction],
                                             feed_dict=feed_dict)
    ...
    def _train_with_fixed_number_of_steps_and_random_feed_dict(self):
        ...
        with tf.Session(graph=self.graph) as sess:
            ...
            for step in range(self.max_steps):
                feed_dict = self.model_input.fill_feed_dict_in_order(self.input_pl, self.target_pl, self.batch_size,
                                                                     step)
                loss_value, predicted = sess.run([self.model_content.optimize, self.model_content.prediction],
                                                 feed_dict=feed_dict)
                if (step % self.test_step == 0):
    ...
    def _test_last_batch(self, sess, num_batches):
        if num_batches * self.batch_size < self.model_input.get_number_datapoints():
            feed_dict = self.model_input.fill_last_feed_dict_in_order(self.input_pl, self.target_pl, self.batch_size,
                                                                      num_batches)
            predicted = sess.run(self.model_content.prediction, feed_dict=feed_dict)
            self._add_predicted_to_predictions(predicted)
    ...
    def get_target_and_predictions_df(self):
        if self.fill_output_timegaps: self.model_output.fill_time_gaps_in_target_and_predictions_using_time_format(
    ...
    def get_prediction_df(self):
        if self.fill_output_timegaps: self.model_output.fill_time_gaps_in_predictions_using_time_format(
            self.current_input_target_maker.time_format)
