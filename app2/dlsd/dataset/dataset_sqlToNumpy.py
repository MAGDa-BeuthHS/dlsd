...
def sqlToNumpy_pivotAndSmooth(inputFile,specifiedSensors,sensorsOutputPath=None,sensorEfficiency=.98,window = 50):
    '''
...
def sqlToNumpy_allSensorsInAllOutWithTimeOffset(inputFile,
    ...
    debugInfo(__name__,"Creating output data at time offset %d"%timeOffset)
    ...
    debugInfo(__name__,"Removing rows with >0 NaNs")
    data_final_naDropped = data_final.dropna()
    debugInfo(__name__,"From %d total timepoints, %d are being used (%.2f)"%(originalSize,data_final_naDropped.shape[0],(data_final_naDropped.shape[0]/originalSize)))

    #data_final.to_csv("/Users/ahartens/Desktop/Temporary/24_10_16_wideTimeSeriesBelegung.csv")
    if (outputFilePath is not None):
        debugInfo(__name__,"Saving processed file to %s"%(outputFilePath))
...
def sqlToNumpy_allSensorsWithAllTimeOffsetAsInput(inputFile,
    ...
    debugInfo(__name__,"Removing rows with >0 NaNs")
    data_final_naDropped = data_final.dropna()
    debugInfo(__name__,"From %d total timepoints, %d are being used (%.2f)"%(data_wide_all.shape[0],data_final_naDropped.shape[0],(data_final_naDropped.shape[0]/data_wide_all.shape[0])))
