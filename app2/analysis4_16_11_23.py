...
def main(args):
    ...
    config.timeOffsets = [5,10,15,30,45]
    ...
    for indexOutputSensor in range(0,data_df.shape[1]):
        ...
        # set up empty data frame and array for the average MAEof all testing data. first column is names of all the functions
        ...
        for i in range(0,len(config.test_dicts)):
            ...
            if not os.path.exists(currentDir_rslts) : os.makedirs(currentDir_rslts)

            path_allMaesForSensor = os.path.join(current_df_dir,"allMaesForSensor_%d.csv"%data_df.columns.values[indexOutputSensor])
            ...
            for j in range(0,len(methods)):
                ...
                config.path_outputFile = os.path.join(currentDir_rslts,"predictions_%s.csv"%methods[j]['name'])
            ...
            all_maesForSensor.index = np.array([(lambda x:x['name'])(funcDic) for funcDic in methods])
        ...
        avgMaes_df.index = [(lambda x:x['name'])(funcDic) for funcDic in methods]
        avgMaes_df.to_csv(path_avgMaes_df,header = ([(lambda x:"t_%d"%x)(to) for to in config.timeOffsets]))
